...
function displayInitialMessage() {
    const output = document.getElementById('output-text');
    output.innerHTML = 'Welcome to Battleship!\n' +
...
function processNewState() {
    ...
    if (pollData.revisionId < 1) {
        if (pollData.requestToJoin === true) {
            ...
            const choice = confirm('Player ' + pollData.playerTwoID + ' has requested to join your game');
            return returnChoice(choice);
        }
        if (pollData.revisionId === -1) {
            setOpponentID();
            if (pollData.playerOneID === null || pollData.playerTwoID === null) {
                document.getElementById('output-text').innerHTML = 'Waiting for opponent to join...';
                console.log('Waiting for opponent to join...');
            } else {
                document.getElementById('output-text').innerHTML = 'Waiting for '+ opponent + ' to place ships...';
            ...
            if (pollData.activePlayer === player.name) {
                document.getElementById('output-text').innerHTML += 'Your turn!\n';
                document.getElementById('output-text').innerHTML += 'Click on a cell in Target Grid to attack!\n';
            } else {
                document.getElementById('output-text').innerHTML += 'Waiting for ' + opponent + ' to attack...\n';
                updateState();
            }
        }
        return;
...
function startNewGame() {
    ...
    .then(data => {
        ...
        document.getElementById('playing-room').style.visibility = 'visible';
...
document.getElementById('join-random-game').addEventListener('click', joinRandomGame);
