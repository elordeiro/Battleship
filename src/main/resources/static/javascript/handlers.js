...
function placeShip() {
    ...
    if (droppedShip.alignment === "horizontal") {
        for (let j = 0; j < droppedShip.length; j++) {
            const cell = document.getElementById((playerGrid === null || playerGrid === void 0 ? void 0 : playerGrid.id) + "-" + (i + j));
            if (cell) {
        ...
        for (let j = 0; j < droppedShip.length; j++) {
            const cell = document.getElementById((playerGrid === null || playerGrid === void 0 ? void 0 : playerGrid.id) + "-" + (i + j * 10));
            if (cell) {
                cell.dataset.occupied = "true";
            }
        }
    ...
    for (let j = 0; j < droppedShip.length; j++) {
        document.getElementById("player-ships" + "-" + droppedShip.type + "-" + (j + 1)).dataset.placed = "true";
    }
    if (player) {
        player.ships.push(new Ship(droppedShip.type.toUpperCase(), linearTo2D(i), droppedShip.alignment.toUpperCase()));
    }
    droppedShip.type = "none";
}
function handleClickOceanGrid(cellIdx) {
    var _a;
    if (gameStarted) {
        return;
    }
    ...
    if (ship.alignment === "horizontal") {
        for (let j = 0; j < len; j++) {
            const cell = document.getElementById((playerGrid === null || playerGrid === void 0 ? void 0 : playerGrid.id) + "-" + (cellIdx + j));
            cell.className = "grid-item-" + ship.type;
        }
    }
    else {
        for (let j = 0; j < len; j++) {
            const cell = document.getElementById((playerGrid === null || playerGrid === void 0 ? void 0 : playerGrid.id) + "-" + (cellIdx + j * 10));
            cell.className = "grid-item-" + ship.type;
        }
    }
    ...
    Array.from((_a = playerShips === null || playerShips === void 0 ? void 0 : playerShips.childNodes) !== null && _a !== void 0 ? _a : [])
        .filter((cell) => cell.id.includes(ship.type))
...
function handleClickTargetGrid(cellIdx) {
    if (!gameStarted) {
        return;
    }
    ...
    const cell = document.getElementById((targetGrid === null || targetGrid === void 0 ? void 0 : targetGrid.id) + "-" + cellIdx);
...
function handleMouseOverOceanGrid(cellIdx) {
...
function handleMouseOverTargetGrid(cellIdx) {
    if (!gameStarted) {
        return;
    }
    if (pollData.activePlayer !== player.name) {
...
function handleMouseOutOceanGrid(cellIdx) {
    if (gameStarted || pollData.game_over === true) {
        return;
    }
    ...
    if (ship.alignment === "horizontal") {
        for (let j = 0; j < len; j++) {
            const cell = document.getElementById((playerGrid === null || playerGrid === void 0 ? void 0 : playerGrid.id) + "-" + (cellIdx + j));
            if ((cell === null || cell === void 0 ? void 0 : cell.className) === "grid-item-" + ship.type) {
...
function handleMouseOutTargetGrid(cellIdx) {
    if (!gameStarted) {
        return;
    }
    const cell = document.getElementById((targetGrid === null || targetGrid === void 0 ? void 0 : targetGrid.id) + "-" + cellIdx);
