...
function wasSet(ship) {
    ...
    return (cell === null || cell === void 0 ? void 0 : cell.dataset.placed) === "true";
}
// Returns true if ship overlaps with another ship
function overlaps(ship, cellIdx) {
    ...
    if (ship.alignment === "horizontal") {
        for (let j = 0; j < len; j++) {
            ...
            if ((cell === null || cell === void 0 ? void 0 : cell.dataset.occupied) === "true") {
                return true;
            }
        }
    }
...
function shipOutOfBounds(ship, cellIdx) {
    if (ship.alignment === "horizontal") {
...
function removeShipFromShips(ship) {
    const len = getShipLength(ship.type);
    for (let j = 1; j <= len; j++) {
        const shipCell = document.getElementById("player-ships" + "-" + ship.type + "-" + j);
        if (shipCell !== null) {
